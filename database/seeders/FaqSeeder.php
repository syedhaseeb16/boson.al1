...
class FaqSeeder extends BaseSeeder
{
    ...
    public function run()
    {
        ...
        foreach ($categories as $locale => $categoryItem) {
            foreach ($categoryItem as $index => $value) {
                $value['order'] = $index;
                $category = FaqCategory::create($value);
                ...
                if ($locale !== 'en_US') {
                    $originValue = LanguageMeta::where([
                        ...
                        'reference_type' => FaqCategory::class,
        ...
        foreach ($data as $locale => $faqItem) {
            foreach ($faqItem as $index => $value) {
                $faq = Faq::create($value);
                ...
                if ($locale !== 'en_US') {
                    $originValue = LanguageMeta::where([
                        'reference_id'   => $index + 1,
                        'reference_type' => Faq::class,
                    ])->value('lang_meta_origin');
                }

                LanguageMeta::saveMetaData($faq, $locale, $originValue);
            }
        }
    }
}
