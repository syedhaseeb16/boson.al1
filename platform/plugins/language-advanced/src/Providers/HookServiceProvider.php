...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        if (!empty($defaultLanguage)) {
            ...
            add_filter(BASE_FILTER_BEFORE_GET_SINGLE, [$this, 'getDataLanguageBeforeShow'], 1134, 4);
    ...
    public function getDataLanguageBeforeShow($data, $model)
    {
        ...
        if ((is_in_admin() && Language::getCurrentAdminLocaleCode() == Language::getDefaultLocaleCode()) ||
            (!is_in_admin() && Language::getCurrentLocaleCode() == Language::getDefaultLocaleCode())
        ) {
            return $data;
        }
        ...
        $language = DB::table($translationTable)
            ...
            ->where($translationTable . '.lang_code', Language::getCurrentAdminLocaleCode())
            ->first();
        ...
        if ($language) {
            ...
            $data = $data->where($translationTable . '.lang_code', Language::getCurrentAdminLocaleCode());
        }
    ...
    public function checkItemLanguageBeforeGetListItem($query, $model)
    {
        ...
        $queryColumns = (array) $query->getQuery()->columns;

        if (!$queryColumns) {
            $query = $query->select([$table . '.*']);
        } else {
            $translatableColumns = LanguageAdvancedManager::getTranslatableColumns($model);

            foreach ($queryColumns as $key => $column) {
                if (!$column instanceof Expression) {
                    if (Str::contains('.', $column)) {
                        $column = Arr::last(explode('.', $column));
                    }

                    if (in_array($column, $translatableColumns)) {
                        Arr::forget($queryColumns, $key);
                    }
                }
            }

            $query->getQuery()->columns = $queryColumns;
        }

        return $query
            ->addSelect([$translationTable . '.*'])
            ->join($translationTable, $translationTable . '.' . $table . '_id', $table . '.id')
            ->where($translationTable . '.lang_code', $currentLocale);
    }
}
