    ...
    @foreach ($categories->where('parent_id', $parent_id ?? 0) as $category)
        <li class="folder-root open">
            <a href="{{ $canEdit ? route('product-categories.edit', $category->id) : '' }}" class="fetch-data category-name">
                ...
                @endphp
                <span class="badge {{ $badge }} font-weight-bold" data-bs-toggle="tooltip"
                    data-bs-original-title="{{ trans('plugins/ecommerce::product-categories.total_products', ['total' => $category->products_count]) }}">
            ...
            <a href="{{ $category->url }}"
                ...
                data-bs-original-title="{{ trans('plugins/ecommerce::product-categories.view_new_tab') }}">
